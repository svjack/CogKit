...
def get_lora_paths():
    lora_dir = "/share/home/zyx/Code/CogKit/lora_path"  # Changed to relative path
...
def generate_images(
    ...
    if lora_path and lora_path != "None":
        full_lora_path = os.path.join("/share/home/zyx/Code/CogKit/lora_path", lora_path)
...
def infer(
    ...
    # 从OpenAI的Image对象中提取URL或b64_json
    image_results = []
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    refresh_button = gr.Button("刷新Lora文件夹", size="sm")
            with gr.Column():

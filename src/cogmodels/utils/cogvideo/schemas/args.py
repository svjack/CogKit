...
class Args(BaseModel):
    ...
    output_dir: Path = Path(
        f"train_results/{datetime.datetime.now():%Y-%m-%d-%H-%M-%S}"
    )
    ...
    validation_steps: (
        int | None
    )  # if set, should be a multiple of checkpointing_steps
    ...
    validation_images: (
        str | None
    )  # if set do_validation and model_type == i2v, should not be None
    validation_videos: (
        str | None
    )  # if set do_validation and model_type == v2v, should not be None
    gen_fps: int = 15
    ...
    def validate_validation_required_fields(
        ...
        if values.get("do_validation") and not v:
            ...
            raise ValueError(
                f"{field_name} must be specified when do_validation is True"
            )
        return v

    @field_validator("validation_images")
    def validate_validation_images(
        cls, v: str | None, info: ValidationInfo
    ) -> str | None:
        values = info.data
        if (
            values.get("do_validation")
            and values.get("model_type") == "i2v"
            and not v
        ):
            raise ValueError(
    ...
    def validate_validation_videos(
        cls, v: str | None, info: ValidationInfo
    ) -> str | None:
        values = info.data
        if (
    ...
    def validate_validation_steps(
        ...
        if values.get("do_validation"):
            if v is None:
                raise ValueError(
                    "validation_steps must be specified when do_validation is True"
                )
            if (
                values.get("checkpointing_steps")
                and v % values["checkpointing_steps"] != 0
            ):
                raise ValueError(
                    "validation_steps must be a multiple of checkpointing_steps"
                )
        return v
    ...
    def validate_train_resolution(
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    "train_resolution must be in format 'frames x height x width'"
                )
            raise e
    ...
    def validate_mixed_precision(cls, v: str, info: ValidationInfo) -> str:
        if (
            v == "fp16"
            and "cogvideox-2b"
            not in str(info.data.get("model_path", "")).lower()
        ):
            logging.warning(
    ...
    def parse_args(cls):
        ...
        # Validation
        parser.add_argument(
            "--do_validation", type=lambda x: x.lower() == 'true', default=False
        )

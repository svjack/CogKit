...
def preprocess_video_with_resize(
    ...
    video_reader = decord.VideoReader(
        uri=video_path.as_posix(), width=width, height=height
    )
    ...
    if video_num_frames < max_num_frames:
        ...
        indices = list(
            range(0, video_num_frames, video_num_frames // max_num_frames)
        )
...
def preprocess_video_with_buckets(
    ...
    nearest_res = min(
        resolution_buckets,
        key=lambda x: abs(x[1] - frames.shape[2]) + abs(x[2] - frames.shape[3]),
    )

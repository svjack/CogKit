...
class BaseI2VDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        train_resolution_str = "x".join(
            str(x) for x in self.trainer.args.train_resolution
        )
        ...
        prompt_embedding_path = prompt_embeddings_dir / (
            prompt_hash + ".safetensors"
        )
        encoded_video_path = video_latent_dir / (video.stem + ".safetensors")

        if prompt_embedding_path.exists():
            prompt_embedding = load_file(prompt_embedding_path)[
                "prompt_embedding"
            ]
            logger.debug(
            ...
            save_file(
                {"prompt_embedding": prompt_embedding}, prompt_embedding_path
            )
            logger.info(
...
class I2VDatasetWithBuckets(BaseI2VDataset):
    ...
    @override
    def preprocess(
        self, video_path: Path, image_path: Path
    ) -> tuple[torch.Tensor, torch.Tensor]:
        video = preprocess_video_with_buckets(
            video_path, self.video_resolution_buckets
        )
        image = preprocess_image_with_resize(
            image_path, video.shape[2], video.shape[3]
        )
        return video, image

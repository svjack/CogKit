...
from cogmodels.utils.cogvideo.constants import LOG_LEVEL, LOG_NAME
from cogmodels.utils.cogvideo.datasets import (
    I2VDatasetWithResize,
    T2VDatasetWithResize,
)
from cogmodels.utils.cogvideo.datasets.utils import (
    load_images,
    load_prompts,
    load_videos,
...
from cogmodels.utils.cogvideo.schemas import Args, Components, State
from cogmodels.utils.cogvideo.utils import (
...
class Trainer:
    ...
    def _init_distributed(self):
        ...
        mixed_precision = (
            ...
            else self.args.mixed_precision
        )
        report_to = (
            None
            if self.args.report_to.lower() == "none"
            else self.args.report_to
        )
        ...
        if self.UNLOAD_LIST is None:
            ...
            for name in self.UNLOAD_LIST:
                if name not in self.components.model_fields:
                    raise ValueError(
                        f"Invalid component name in unload_list: {name}"
                    )
    ...
    def prepare_dataset(self) -> None:
        ...
        logger.info(
            "Precomputing latent for video and prompt embedding ... Done"
        )
    ...
    def prepare_trainable_parameters(self):
        ...
        for attr_name, component in vars(self.components).items():
            if hasattr(component, "requires_grad_"):
                if (
                    self.args.training_type == "sft"
                    and attr_name == "transformer"
                ):
        ...
        self.__move_components_to_device(
            dtype=weight_dtype, ignore_list=ignore_list
        )
    ...
    def prepare_optimizer(self) -> None:
        ...
        if self.args.train_steps is None:
            self.args.train_steps = (
                self.args.train_epochs * num_update_steps_per_epoch
            )
        ...
        total_training_steps = (
            self.args.train_steps * self.accelerator.num_processes
        )
        num_warmup_steps = (
    ...
    def prepare_for_training(self) -> None:
        ...
        self.args.train_epochs = math.ceil(
            self.args.train_steps / num_update_steps_per_epoch
        )
        self.state.num_update_steps_per_epoch = num_update_steps_per_epoch

    def prepare_for_validation(self):
        validation_prompts = load_prompts(
            self.args.validation_dir / self.args.validation_prompts
        )

        if self.args.validation_images is not None:
            validation_images = load_images(
                self.args.validation_dir / self.args.validation_images
            )
        else:
            validation_images = [None] * len(validation_prompts)

        if self.args.validation_videos is not None:
            validation_videos = load_videos(
    ...
    def prepare_trackers(self) -> None:
        ...
        self.accelerator.init_trackers(
            tracker_name, config=self.args.model_dump()
        )
    ...
    def train(self) -> None:
        ...
        logger.info(
            f"Memory before training start: {json.dumps(memory_statistics, indent=4)}"
        )
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            logger.debug(
                f"Starting epoch ({epoch + 1}/{self.args.train_epochs})"
            )
    ...
    def validate(self, step: int) -> None:
        ...
        for i in range(num_validation_samples):
            ...
            for i, (artifact_type, artifact_value) in enumerate(
            ...
            for key, value in list(artifacts.items()):
                ...
                if (
                    artifact_type not in ["image", "video"]
                ...
                if artifact_type == "image":
                    ...
                    export_to_video(
                        artifact_value, filename, fps=self.args.gen_fps
                    )
            ...
            for tracker in accelerator.trackers:
                if tracker.name == "wandb":
                    image_artifacts = [
                        ...
                        if isinstance(artifact, wandb.Image)
                    ]
                    video_artifacts = [
                        artifact
                        for artifact in all_artifacts
                        if isinstance(artifact, wandb.Video)
                    ]
                    tracker.log(
                        {
                            tracker_key: {
        ...
        memory_statistics = get_memory_statistics()
        logger.info(
            f"Memory after validation end: {json.dumps(memory_statistics, indent=4)}"
        )
    ...
    def __get_training_dtype(self) -> torch.dtype:
        if self.args.mixed_precision == "no":
            ...
            raise ValueError(
                f"Invalid mixed precision: {self.args.mixed_precision}"
            )
    ...
    def __maybe_save_checkpoint(
        self, global_step: int, must_save: bool = False
    ):
        if (

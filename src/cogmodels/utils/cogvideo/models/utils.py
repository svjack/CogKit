...
def register(
    model_name: str, training_type: Literal["lora", "sft"], trainer_cls: Trainer
):
    ...
    if model_name not in SUPPORTED_MODELS:
        ...
        raise ValueError(
            f"Training type {training_type} already exists for model {model_name}"
        )
...
def get_model_cls(
    model_type: str, training_type: Literal["lora", "sft"]
) -> Trainer:

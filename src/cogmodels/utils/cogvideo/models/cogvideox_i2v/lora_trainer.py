...
from cogmodels.utils.cogvideo.schemas import Components
from cogmodels.utils.cogvideo.trainer import Trainer
from cogmodels.utils.cogvideo.utils import unwrap_model
...
class CogVideoXI2VLoraTrainer(Trainer):
    ...
    def compute_loss(self, batch) -> torch.Tensor:
        ...
        if patch_size_t is not None:
            ...
            first_frame = latent[
                :, :, :1, :, :
            ]  # Get first frame [B, C, 1, H, W]
            latent = torch.cat(
                [first_frame.repeat(1, 1, ncopy, 1, 1), latent], dim=2
            )
        ...
        prompt_embedding = prompt_embedding.view(batch_size, seq_len, -1).to(
            dtype=latent.dtype
        )
        ...
        latent_noisy = self.components.scheduler.add_noise(
        ...
        vae_scale_factor_spatial = 2 ** (
            len(self.components.vae.config.block_out_channels) - 1
        )
    ...
    def prepare_rotary_positional_embeddings(
        ...
        grid_height = height // (
            vae_scale_factor_spatial * transformer_config.patch_size
        )
        grid_width = width // (
            vae_scale_factor_spatial * transformer_config.patch_size
        )

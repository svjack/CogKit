...
def to_torch_tensor(state_dict, return_empty_tensor=False, bfloat16=False):
    """
    ...
    if bfloat16:
        # Convert weights to bfloat16 format
        print("Converting weights to bfloat16 format...")
        for key in list(torch_state_dict.keys()):
            if isinstance(torch_state_dict[key], torch.Tensor):
                # Only convert floating point tensors, leave integer tensors as is
                if torch_state_dict[key].dtype in [torch.float32, torch.float16, torch.float64]:
                    torch_state_dict[key] = torch_state_dict[key].to(torch.bfloat16)
        print("Conversion to bfloat16 completed.")
...
def convert_zero_checkpoint_to_fp32_state_dict(
    ...
    weights_name = (
        "diffusion_pytorch_model.safetensors" if safe_serialization else "pytorch_model.bin"
    )
    if max_shard_size is not None:
    ...
    if state_dict_split.is_sharded:
        ...
        save_index_file = (
            ...
            if safe_serialization
            else "diffusion_pytorch_model.bin.index.json"
        )
